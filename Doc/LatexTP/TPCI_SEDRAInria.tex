...
\newcommand{\hmwkDueDate}{Thursday,\ April\ 28,\ 2016} % Due date
\newcommand{\hmwkClass}{Continuous Integration Seminar} % Course/class
\newcommand{\hmwkClassTime}{13h30} % Class/lecture time
\newcommand{\hmwkClassInstructor}{\url{https://sed-ncy.inria.fr}} % Teacher/lecturer
\newcommand{\hmwkAuthorName}{Laurent Pierron and Emmanuel Jeanvoine} % Your name
...
\author{\textsf{\hmwkAuthorName}\\ (All credits to Maurice Br\'{e}mond, Ga\"{e}tan Harter, David Parsons\\ from SED INRIA Rh\^{o}ne-Alpes for the orginal work)}
...
\begin{itemize}
\item \texttt{Git} (\texttt{sudo apt-get install git} | \texttt{sudo yum install git})
\item A JDK (\texttt{sudo apt-get install openjdk-7-jdk} | \texttt{sudo yum install java-1.7.0-openjdk})
\item \texttt{Maven} (\texttt{sudo apt-get install maven} | \texttt{sudo yum install apache-maven})
\end{itemize}

You also need an account on both \url{gforge.inria.fr} and \url{ci.inria.fr} and to be a member of the \texttt{tpcisedncy} project on both platforms.

\section{Setup}

\subsection{Create a personal copy of the git repository from INRIA forge and clone it}
First, you will create your personal \texttt{git} repository on the INRIA forge as a branch of the main repository for the \texttt{tpcisedncy} project~:
\begin{itemize}
\item Go to \url{https://gforge.inria.fr/projects/tpcisedncy/}
\item Click on the \textbf{\texttt{CODE SOURCE}} or \textbf{\texttt{SCM}} tab
\item Click on \textbf{\texttt{Request a personal repository}}
...
\begin{lstlisting}
$ git clone git+ssh://<yourlogin>@scm.gforge.inria.fr/gitroot/tpcisedncy/
    users/<yourlogin>.git
$ cd <yourlogin>/java
\end{lstlisting}
...
\subsection{Testing your code locally}
...
%--------------------------------------
\subsection{Testing your code remotely on ci.inria.fr}

\subsubsection{Log into the project's Jenkins instance}
\begin{itemize}
...
\subsubsection{Create a new Jenkins item}
From our \texttt{Jenkins} dashboard page (\url{https://ci.inria.fr/tpcisedncy/})~:
\begin{itemize}
...
\begin{lstlisting}
https://scm.gforge.inria.fr/anonscm/git/tpcisedncy/users/<yourlogin>.git
\end{lstlisting}
\end{itemize}
\end{homeworkProblem}
...
To do so, edit \texttt{tpcisedncy/java/src/main/java/fr/inria/sed/Sphere.java}.
...
Why does the test in \texttt{SphereTest.java} fail ?\footnote{This is because of \emph{floating point arithmetics rounding errors}}.
...
\texttt{yourpath/tpcisedncy/java/target/site/cobertura}
...
\subsection{Make your own custom style checker}

\begin{itemize}
\item Retrieve a local copy of google checks (name it \texttt{custom\_checks.xml})
\begin{lstlisting}
$ wget -O custom_checks.xml http://tinyurl.com/z9pfdlx
\end{lstlisting}
\item Tell maven to use this config file by changing the value of variable \texttt{checkstyle.config.location}:
\begin{lstlisting}
$ pwd
yourpath/tpcisedncy/java
$ mvn checkstyle:checkstyle -Dcheckstyle.config.location=custom_checks.xml
\end{lstlisting}
\item Modify the file \texttt{custom\_checks.xml} so that it accepts single character parameter names.
...
\texttt{checkstyle:checkstyle -Dcheckstyle.config.location=custom\_checks.xml}
...
\begin{itemize}
\item \texttt{setuptools} packages your project
\item \texttt{unittest} to write tests (standard library)
\item \texttt{nose} tools run tests and format the results
\item \texttt{mock} allow replacing function or objects during tests
\item \texttt{pylint} provide static code analysis and style checking
\item \texttt{pep8} verifies code compliancy to \texttt{Python} PEP8 style convention
\item \texttt{tox} is a virtual environment based test automation tool.
\end{itemize}
...
\begin{itemize}
\item Try the \texttt{sphere} script with a \texttt{radius} as argument.
\end{itemize}
...
\begin{lstlisting}
$ sudo -H pip install --upgrade  nose nosexcover
\end{lstlisting}
...
\begin{itemize}
\item From our \texttt{Jenkins} dashboard page (\url{https://ci.inria.fr/tpcisedncy/}), click \textbf{\texttt{New Item}} in the menu on the left
...
\subsubsection{Jenkins Job Configuration}

\includegraphics[height=20pt]{configure}

In the new item's \textbf{\texttt{Configure}} page (which you will be redirected
to after clicking \textbf{\texttt{OK}}).
...
\item Click on \textbf{\texttt{Apply}}
\end{itemize}


\subsubsection{Inria forge server git hook configuration}

Then on the INRIA forge server, create the post-commit hook \texttt{post-receive}~:

\begin{lstlisting}
$ ssh <yourforgelogin>@scm.gforge.inria.fr
$ cd /gitroot/tpcisedncy/users/<yourforgelogin>.git/hooks
$ touch post-receive
...
\begin{lstlisting}[breaklines=true]
$ cp /gitroot/tpcisedncy/users/bremond.git/hooks/post-receive /gitroot/tpcisedncy/users/<yourforgelogin>.git/hooks
\end{lstlisting}
...
http://ci.inria.fr/tpcisedncy/git/notifyCommit?url=https://scm.gforge.inria.fr/anonscm/git/tpcisedncy/users/<yourforgelogin>.git
\end{lstlisting}

After every commit, INRIA forge git server will contact \texttt{ci.inria.fr} and tell it to build the projects that use your repository. And so trigger your project build.

\subsubsection{Exercise 6 - Jenkins Configuration}

Add a new build step using \textbf{\texttt{Virtualenv Builder}},
in the job configuration on Jenkins.

\begin{description}
  \item[Note] Using Virtualenv Builder will put the code execution in a separated
    \texttt{Python} \texttt{virtualenv} environment.  This will give a clean
    \texttt{Python} installation and allow installing packages for this build .
\end{description}

Now in this field, you inform \texttt{Jenkins} how to build and test the project.
\begin{lstlisting}
# Install dependencies
pip install --upgrade  nose nosexcover
...
\subsubsection{Re-factoring error}
Why does the test fail?

Because of floating point \emph{arithmetic rounding errors}.
...
also meaningful. It is often referred to as "code coverage".
...
----------------------------------------------------------------------
XML: /home/harter/work/tpcisedncy/python/nosetests.xml
Name                  Stmts   Miss Branch BrPart  Cover   Missing
-----------------------------------------------------------------
tp_ci_sed.py              1      0      0      0   100%
tp_ci_sed/sphere.py      19      6      2      1    67%   19, 28-32, 36, 35->36
...
\subsubsection{Exercise 9 - Coverage on Jenkins}
...
\begin{itemize}
  \item In the build step, put at the beginning:
\begin{lstlisting}
...
\item You can click on the report to see per-file coverage info.
\end{itemize}
...
\subsubsection{Exercise 10 - Pylint/Pep8 on Jenkins}

You will now run \texttt{pylint} and \texttt{pep8} on \texttt{Jenkins}.
However the problem with \texttt{lint} and \texttt{pep8} commands are that they
return a non zero value when there is a warning.

To prevent that, the return value
will be hidden, because, in this case, it should not be fatal.

\begin{itemize}
\item Add \texttt{lint} and \texttt{pep8} in \texttt{Jenkins} VirtualenvBuilder
  build step as:
\begin{lstlisting}
python setup.py lint | true
python setup.py pep8 | true
\end{lstlisting}
The \verb=' | true'= will silent the return value as they are not fatal steps.
(It's a choice, I have projects where Pylint errors are considered failures).
...
\end{itemize}
\
\subsubsection{Exercise 12 - Tox on Jenkins}
...
\textbf{For me, \texttt{tox} is the most important step of this session.}

It really solves common problems and simplify testing by doing test dependency
management, document in a script the test procedure(s)
and that it also handles many tests environments in one command for the user.

\paragraph{Other versions}
If everything succeeds, and you can test using \texttt{python3} (\texttt{Python}3 is installed in the virtual machine related to \texttt{ci.inria.fr/tpcisedncy}
).
\begin{itemize}
\item Use \texttt{envlist = py27,py32} (for \texttt{Python}3.2)
\item Re-run \texttt{tox}.
\end{itemize}
Tests should fail for \texttt{Python}3, at least for the \texttt{print} statement.
...
When testing, it is useful to replace some part of your application under test 
to prevent its side effect, like connection to an external service, or 
make an \texttt{open} call raise an exception.

It is possible to fill your whole hard-drive to get an \texttt{IOError: No space left on device}
but it might be better to just modify \texttt{open} to simulate it to test your code.

It is also useful to be able to know how an object has been used or a function been called,
the same way as testing what has been printed by your program.

\paragraph{}In this section you will see \texttt{mock} to test the \texttt{main} function
and verify the printed output.

\paragraph{Testing main} In this case, testing the \texttt{main} function ie 
calling the \texttt{sphere} script from command line, means~:
\begin{itemize}
  \item Set the command line arguments to some values~: \texttt{sys.argv}
  \item Verify the printed text~: in files \texttt{sys.stdout} and \texttt{sys.stderr}
\end{itemize}

Install \texttt{mock} dependency~: 
\begin{itemize}
...
\begin{itemize}
    \item \texttt{Mock} class which is an object whose behavior can be
...
\paragraph{Hints~:} Some advises on patch~:
...
\begin{itemize}
\item Add \texttt{ChuckNorris} post build action. It will make your code famous and better.
\end{itemize}
...
\texttt{sudo apt-get install cmake gcc g++ libcppunit-dev cppcheck gcovr lcov}
\end{itemize}
...
\begin{lstlisting}
$ git clone \
  git+ssh://<yourlogin@scm.gforge.inria.fr/gitroot/tpcisedncy/users/<yourlogin>.git
$ cd tpcisedncy/cxx
\end{lstlisting}
...
\\
In our case, we want the results to be displayed as soon as possible, so we choose to launch the build after a \texttt{post-commit} hook\footnote{It is explained in \url{ci.inria.fr} documentation in the FAQ}~:
...
\texttt{tpcisedncy/c++/src/Sphere.cpp} and have a look at the code of the method \texttt{Sphere::volume()}.
...
\texttt{tpcisedncy/c++/src/tests/testTP.cpp}
...
\item In \texttt{Jenkins}~:
\begin{itemize}
\item Add the scan for compiler warnings in the \textbf{\texttt{Post build Action}} of your project.
\item In the \textbf{\texttt{Execute shell}} section, comment the line \texttt{make test}. This is necessary because as the test is failing, the parsing will not be done by \texttt{Jenkins}.
\end{itemize}
\item Commit the code in order to check the parsing done by \texttt{Jenkins}
\item If it goes well, then uncomment the \texttt{make test} line.
...
order to present parseable results to \texttt{Jenkins}.
...
Remark~:
\begin{itemize}
\item You can see all \texttt{make} targets using~: \texttt{make help}.
\end{itemize}
% $
...
\item On the \texttt{Jenkins} side~:
\begin{itemize}
\item Modify the build process in \textbf{\texttt{Execute shell}} to add coverage target
\item In \textbf{\texttt{Post-Build Actions}}, add \textbf{\texttt{Publish Cobertura Report}} with the \texttt{Cobertura xml} report pattern set to~:\\
\texttt{**/coverage.xml}
\end{itemize}
\item Commit
\item Generate and visualize the corresponding coverage report.
...
\subsection{Exercise 3bis (optional)}

This is optional, if you prefer, go directly to the static analysis section in page ~\pageref{sec:staticanalysis}.

Let's check if it works with more than one \texttt{C++} class.

Use another development branch to add a new class \texttt{Alphabet} to our project~:
\begin{lstlisting}
...
TODO~:
\begin{itemize}
\item Resolve git merge conflicts (keep the flags you added, use the new library generation dependency)
\item Generate and visualize the corresponding coverage report
...
\texttt{Sphere} objects \texttt{p} is composed of the \texttt{Sphere} \texttt{s1} and the \texttt{Sphere} 
...
\texttt{cppcheck} is a command-line tool dedicated to static \texttt{C/C++} code analysis. It tries to detect bugs that your \texttt{C/C++} compiler does not see. It is versatile, and can check non-standard code including various compiler extensions, inline assembly code, etc. Its internal preprocessor can handle includes, macros, and several preprocessor commands.
...
Beware~: The results we obtain depend on the version of \texttt{cppcheck} !

On the virtual machine configured for \texttt{ci.inria.fr/tpcisedncy}, the latest (\texttt{1.71}) \texttt{cppcheck} is installed.
\clearpage
\subsection{Exercise 4}
TODO~:
\begin{itemize}
\item Test the command given below on your project locally
\item Install this check on \texttt{Jenkins}~:
\begin{itemize}
\item Modify the \textbf{\texttt{Build}} section \textbf{\texttt{Execute shell}} field to call \texttt{cppcheck}
\item Add a \textbf{\texttt{Post-build Action}} for publishing \texttt{cppcheck} results. 
\end{itemize}
Beware~: change the \texttt{cppcheck} command argument so it generates an \texttt{XML} output (use \texttt{--xml} option) and redirect the standard error to a file named
  \texttt{cppcheck-result.xml}.\\
\texttt{Jenkins} can read this file if configured properly and if the appropriate \textbf{\texttt{post-build}} action is set.
